...
namespace no_streaming
{
		...
		const float* p_start_rotation_quat) noexcept
	{
		...
		// the component currently holds the previewed final state — the
		// same dance the Z6 stress suite does: serialize the after state,
		// restore the start state, then let the command's Execute
		// capture the true before and apply the after
