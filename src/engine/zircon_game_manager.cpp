...
void zircon_game_manager::initialize_input(void) noexcept
{
	...
	// GLFW input callbacks are NOT installed from here: GLFW is a static
	// library and every module owns a private copy of its global state, so
	// a glfwSet*Callback from game.ktk writes into this module's
	// never-initialized copy and is silently dead (the same disease as the
	// imgui backend — the window lives in kotek.exe's copy). The exe-side
	// window (kotek.core.window.glfw, task K26) installs the callbacks on
	// the live copy and feeds Get_Input() directly, unconditionally — the
	// editor gate that used to skip the installation is gone. The own
	// Win32 window backend (task K17) wires its input path in phase 2
	// through the window's WndProc + the win32 input backend.
}
