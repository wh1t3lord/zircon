...
zircon_session_game::zircon_session_game(kotek::uint8_t id) :
	...
	m_id{id},
	m_render_graph_id{static_cast<kotek::uint8_t>(-1)},
	m_p_main_manager{}, m_p_console{}, m_p_world{},
	m_name{"not_inited"}
{
}
...
{
	KOTEK_ASSERT(
		!this->m_was_initialized,
		"you forgot to call shutdown method"
	);

#ifdef KOTEK_DEBUG
	KOTEK_ASSERT(
		this->m_was_allocated_by_manager
			? this->m_was_destroyed_by_manager
			: !this->m_was_destroyed_by_manager,
		"if inited_by_manager was true it means this instance "
	    "must be "
		"destroyed through calling from manager but you can't "
	    "by yourself call "
...
const char* zircon_session_game::get_session_name(void
) const noexcept
{
	return this->m_name.c_str();
}

eZirconSessionType zircon_session_game::get_type(void
) const noexcept
{
...
{
	KOTEK_ASSERT(
		session_name.empty() == false,
		"you must pass a reasonable name please!"
	);
	KOTEK_ASSERT(p_current_world, "must be a valid world!");
	KOTEK_ASSERT(
		this->m_id != _kInvalidSessionID ? this->m_id == id
										 : true,
		"you must pass a same id as you passed in ctor. "
	    "Otherwise it means "
		"that you won't obtain a right session by id. Default "
	    "ctor was created "
		"for handling rare case where a user needs pure "
	    "deferred "
		"initialization, but when it comes is unknown... So "
	    "generally for "
		"validation of your ::initialize calling you have to "
	    "construct this "
		"instance with ctor that accepts id in order to "
	    "prevent some "
		"misclicking behaviour"
	);
	KOTEK_ASSERT(
		p_current_world->get_ecs_context(),
		"it is supposed that world will have created context "
	    "otherwise world without any entities is like no world "
	    "and then what is the sense of such state?!"
	);
	KOTEK_ASSERT(
		this->m_was_initialized == false,
		"you should call shutdown and then initialize or "
	    "reinitialize method "
		"for calling twice. "
		"Otherwise why do you need to call this method again?"
	);
	KOTEK_ASSERT(
		p_main_manager, "you must pass a valid main manager!"
	);
	...
	if (!this->m_was_initialized)
	{
		...
		{
			// backends without render graphs (NRI phase 1, task K11/Z5)
			// keep the invalid id — there is nothing to initialize
			constexpr kotek::uint8_t k_invalid_render_graph_id =
				static_cast<kotek::uint8_t>(-1);

			if (this->m_render_graph_id != k_invalid_render_graph_id &&
				this->m_p_console)
			{
				...
				// supposed that game session is 'optimized'
				// session so it is expected that you pass
				// render graph for shipping aka release so
				// there's no point for implementing features
				// like changing render graphs and etc it is
				// useful only for SDK development cycle/build
#ifdef KOTEK_USE_SDK_IMGUI
...
#ifdef KOTEK_DEBUG
		KOTEK_MESSAGE(
			"initialized game session {} {} with world {}",
			session_name,
			id,
			p_current_world->get_id()
		);
#endif
	}
}

kotek::uint8_t zircon_session_game::get_render_graph_id(void
) const noexcept
{
	return this->m_render_graph_id;
}

void zircon_session_game::set_render_graph_id(kotek::uint8_t id
) noexcept
{
...
zircon_world* zircon_session_game::get_world(void
) const noexcept
{
...
{
	KOTEK_ASSERT(
		this->m_imgui_ui_elements.empty(),
		"already initialized!"
	);
...
void zircon_session_game::try_to_initialize_render_graph(void
) noexcept
{
#ifdef KOTEK_USE_SDK_IMGUI
	if (!this->m_was_render_graph_initialized)
	{
		KOTEK_ASSERT(
			this->m_p_console,
			"supposed that you should initialize m_p_console "
		    "field so early "
			"calling"
		);
