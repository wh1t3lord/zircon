...
p_main_manager->Get_ImguiWrapper();
        ...
        {
            if
(p_wrapper_imgui->BeginTabBar("ZirconComponentInput"))
            {
                if (p_wrapper_imgui->BeginTabItem("info"))
                {
                    ...
                    // TODO: implement showing all devices that
connected to ps
                    // for each devices print all necessary
information if (this->m_p_input_manager)
                    {
...
\n\t%.3f (delta) "
                                              "\n\t%.3f
(delta*sens)", this->m_p_input_manager->Get_ControllerData(
...
bool zircon_component_input::is_key_holding(
	kotek::core::eInputAllKeys key, unsigned char frames
) const
{
	KOTEK_ASSERT(
		this->m_p_input_manager,
		"you forgot to initialize this class, probably wrong "
		"constructor was "
		"used?"
	);
	...
	if (this->m_p_input_manager)
	{
		return this->m_p_input_manager->Is_KeyHolding(
			this->m_p_input_manager->Get_ControllerTypeByKey(key
		    ),
			key,
			frames
		);
...
{
	KOTEK_ASSERT(
		p_input_manager, "you can't pass invalid pointer!"
	);
...
float zircon_component_input::get_delta_x(
	kotek::core::eInputControllerType type
) const
{
	if (this->m_p_input_manager)
	{
		switch (type)
		{
		case kotek::core::eInputControllerType::
			kControllerMouse:
		{
			float result =
				this->m_p_input_manager->Get_ControllerData(
