...
public:
	...
	void draw_imgui(Kotek::Core::ktkMainManager* main_manager
	) noexcept override;
	kotek::json::value serialize(void) noexcept override;
	void deserialize(const kotek::json::value& data
	) noexcept override;
...
)
{
	...
	#ifdef KOTEK_DEBUG
	frustum
		[ZIRCON_DEF_GAME_ZIRCON_COMPONENT_FRUSTUM_FIELD_M_COMPONENT_TYPE] =
...
)
{
	...
	result.set_enabled(
		frustum
			.at(ZIRCON_DEF_GAME_ZIRCON_COMPONENT_FRUSTUM_FIELD_M_COMPONENT_TYPE
	        )
			.as_bool()
	);
