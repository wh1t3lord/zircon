...
| `localization_editor_language` | string (language tag) | `en` | active language of the EDITOR localization instance (task Z22): names the `data_game/configs/locale/editor/<tag>.json` table; lookups fall back to `en`, then echo the key itself; runtime-switchable through `zircon_localization_manager::set_language` (persisted at the next save); tags are 1–16 chars of `[a-zA-Z0-9_-]` (the tag becomes a file name) |
| `localization_game_language` | string (language tag) | `en` | active language of the GAME localization instance (`data_game/configs/locale/game/<tag>.json`) — layer-3 in-game text, same fallback chain; switches at load |
