...
|---|---|---|---|
| `render_passes_editor` | string (comma list) | `present,imgui` | the editor session's render pass set, in execution order; names come from the pass factory registry |
| `render_passes_game` | string (comma list) | `present` | the game session's render pass set (gains `model_static` when that pass lands) |
