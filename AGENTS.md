...
| Z13 | CI/CD green on GitHub + three linkage configurations in CI | in-progress (2026-07-25) | workflows build/tests exist and run on every push; build.yml already compiles the THREE configurations the owner requires (default / full static / full dynamic as continue-on-error per K18) and tests.yml runs the real engine boot (`--no_splash --kotek_frames=30`, `KOTEK_ASSERT_STDERR_ROUTING=ON`). ALIGNED WITH KOTEK K5 (2026-07-25): `restore-keys` vcpkg cache fallback in both workflows (key bump → previous tree restored, only changed ports rebuild — the cold-cache failure class that bit kotek's tests on 8670cd7), configure retry ×3 with tee'd `configure-attempt<N>.log` + `build-output.log` (was single-shot, blind), `contents: write` + clone-based `ci-logs` publishing on failure in both workflows (job logs/artifacts are admin-only for the owner — the public ci-logs branch is the read path), timeouts 120→180. Kotek-side CI is fully green (build all 4 legs on 8670cd7); zircon legs verify with this push |
