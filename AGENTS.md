...
> Last updated: 2026-09-23 (task Z24 B3b: the geometry seam + the first
> real NRI draw — the NRI meshlet cluster pass; see the Z24 row and the
> K11 geometry-seam paragraph).
...
9. **Memory, streaming, cache (the standing engineering bar).** Memory is a
   ...
   **Data-oriented DS&As, never STL-shaped ones (Jebrim's DOD
   philosophy, owner directive 2026-09-12):** we do NOT replicate
   textbook containers/algorithms — we pick DOD-native structures per
   use case: GRIDS (fixed 2D/spatial cells), POOLS (dense storage +
   free-list of handles, never per-node allocation), AoSoA (arrays of
   small SIMD-sized bundles — the default multi-variable layout when a
   hot loop touches several fields), SoA (one array per field when a
   system consumes a single field of many), RING BUFFERS (bounded
   producer/consumer queues). Never trees, never hash maps (the
   lookup-table-on-vector rule 2 already covers associations; a hash
   map needs the written justification). The prefetcher supports only a
   handful of independent arrays before it chokes — prefer AoSoA over
   many parallel SoA streams, and remember the reason SoA/AoSoA win is
   SIMD, not ideology: layout exists to feed vectorized math (frustum
   tests, plane clipping, matrix batches). **Byte flags and smallest
   types are the DEFAULT** (`uint8_t` for states/flags/priorities/small
   counts, `uint16_t` before `uint32_t`, `int` only with a measured
   reason) — every struct field's width is a budget decision, not a
   habit. Existing reference shapes: the ECS dense per-component arrays
   (SoA), the chunk/compound tables (flat POD arrays), the cancel
   arbiter (16-entry poll, u8 priorities), the kpack entry table (fixed
   45-byte records in one array).
...
│                                + zircon_resource_manager (MT — the worker's text loads do real
│                                streamed IO through ktkIFileSystem's B3 streaming API, kotek K25) + tests
...
│             zircon_kpacker     .kpack host tool (filesystem plan B2b): pack/list/add/remove/verify,
│                                links kotek.core.filesystem.pack (the shared encoder/reader)
...
| Z8 | Track codebase TODOs | open | **Re-censused 2026-09-03**: 77 markers in `src/` (66 code + 11 CMake boilerplate), down ~41% from the ~130 of 2026-07-21 — the command-history cluster is RESOLVED (Z6 rewrite; only the binary-journal todo in `zircon_command_definitions.h:73` remains). Live unimplemented traps (16): editor session Serialize/Deserialize assert unconditionally but are reachable via LoadScene/SaveScene console commands (`zircon_session_editor.cpp:162,391`), resource formats `.ktx/.ogg/.km` + unload (`zircon_resource_manager.cpp:283-533`), profiler stubs (`main_game_dll.cpp:100,168`). Dormant `#error`s (8): 4 ENTT factory branches + 4 non-GLFW/SDL window-lib branches. Dead-block deletion candidate ~200 lines: `zircon_game_manager.cpp:2765-2938` (the commented console registrations AGENTS.md used to cite at ~:2101 — line drift). DONE 2026-09-03: the empty PICO branches — `zircon_editor_ui_state::initialize` (the empty-inspector root cause) and `zircon_factory::get_all_components_of_entity` (silent empty-result defect, masked by the Serialize trap). RESOLVED 2026-09-04: C1 console component ops — the PICO command branches themselves were filled 2026-09-03, but the five lambdas stayed unreachable from every UI button until Z20's uint32 marshaling fix (the PICO migration had re-typed them to `kotek::entity_t`; §5's exact-type note). Recommended order: C3 scene serialize bridge/delete → C2 dead-block deletion → C4 graceful format rejection. FILESYSTEM B0 (kotek K25, 2026-09-04): the kotek filesystem no longer asserts on missing files (graceful false + one warning) and `Get_FileSize(path)` is implemented — zircon-side fallout landed: `zircon_gltf_loader.cpp::gltf_read_file` dropped the Is_Exists + alias-layer `file_size` TOCTOU pair for the interface `Get_FileSize(path)`, the three shader-blob readers' stale "native read path asserts" comments updated (the Is_Exists guards stay — they suppress the warning by design), scene_metadata's guard needed no change; the `.ktx/.ogg/.km` format traps remain (C4). FILESYSTEM B2b (kotek K25, 2026-09-12): the `.kpack` host tool `zircon_kpacker` landed (pack/list/add/remove/verify, offline wholesale-rewrite mutation, embedded `_kpack_names.json` name sidecar — see kotek's K25 row and `doc/git/en/configuration.md` §Tools); zircon-side boot proof `Zircon_Game.PackBootDispatcherReadsConventionalFolder` pins the boot dispatcher's conventional mount + the pack-first prepend (a same-named file resolves from the PACK by default; the explicit `["Native","Pack"]` config flips it to native-wins). FILESYSTEM B3 (kotek K25, 2026-09-12): the streaming API is real (bounded 8-slot pool, native-CRT/VFM-mapped/pack-block backends, forward-only HDD discipline — see kotek's K25 row); zircon-side integration landed in the resource manager — the `kText` branch reads through Begin/Read/End_Stream (the worker thread's first real IO; chunked into the `ZIRCON_DEF_RESOURCE_MANAGER_STREAM_BUFFER_SIZE` scratch + json parse; missing/empty/oversized fails loud-but-graceful with is_loaded=false), `make_request`'s async contract fixed (view allocation + request-to-handle wiring — the async path was never reachable before), new `get_view` accessor; tests `ResourceManagerLoadTextResourceThroughWorkerStream` (new) + `ResourceManagerLoadTextResourceNoCache` (actualized with content assertions; the fixture's `sizeof(pointer)` 7-byte-write latent bug fixed). The `.ktx/.ogg/.km` format traps remain (C4) — the B3 work did not touch them. FILESYSTEM API-SIMPLICITY (kotek K25's last plan item, 2026-09-12): kotek gained the header-only free-function helpers over `ktkIFileSystem` (`kotek.core.filesystem/include/kotek_filesystem_helpers.h`: `read_file`/`read_json`/`write_json`/`file_size`/`path_for`, reachable through the module umbrella — the contracts, the `KOTEK_DEF_FILESYSTEM_HELPERS_JSON_SCRATCH_SIZE`=64 KB bound and the silence rule are documented there and in kotek's K25 row). The zircon call-site sweep (mechanical, semantic-free — the suites prove it): `zircon_config.cpp` serialize (path_for + write_json: the 15-line serialize+write block → 4 lines) + deserialize (path_for + read_json: the 21-line array_t/Read_File/Create_FromMemory block → 6 lines, the Is_Exists first-boot gate KEPT — absence is normal there), `zircon_scene_metadata.cpp` save (17 → 4) + load (25 → 8, Is_Exists gate kept), the three bgfx shader-blob readers (grid/gizmo_own/model_static: path_for for the `shader_cache/bgfx` root + read_file for the bounded blob read, the p_buffer/buffer_length aliasing dance gone; the Is_Exists pre-pipeline guards stay by design) + model_static's mesh path (path_for), `zircon_gltf_loader.cpp::gltf_read_file` (file_size/read_file; the offset logic kept — `io_used` now advances by the REAL read size, identical to the old file_size advance in every non-race execution). DELIBERATELY LEFT (the helper doesn't clearly shorten, no churn): the command journal (its block IO is the Z6 fstream machinery, not ktkIFileSystem calls; the one Make_Path feeds a u8string conversion into a member cstring), the localization manager (its own loader + 192 KB bound, Z22), the resource manager (already on the B3 stream API directly), the editor imgui pass's imgui.ini path (a single Make_Path, not boilerplate). VERIFIED: main+gfxdev Debug RC 0, boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 82/82 zircon + 260/260 kotek (5 new `Filesystem.test_helpers_*` OK lines), zero new boot-flow warnings, 4 debug_heap(996) residuals (≤6 tolerated), game_config.json roundtrip intact |
...
| Z23 | Fault-tolerant resource defaults chain (B4) | done (2026-09-12) | the filesystem plan's FINAL phase (kotek K25: the overhaul is COMPLETE) — the proven-by-time pattern (Source's magenta checkerboard, Unreal's default material): a resource request resolves through the priority chain (data_user overrides → data_game dirs → mounted packs — the dispatcher machinery, NOT reimplemented) and when NOTHING resolves, the EMBEDDED default for the resource's type answers; a missing/unreadable resource NEVER exit-faults/asserts, the embedded blob is returned with a loud-once log (missing content must be visible, not silent). NEW `src/core/zircon_embedded_defaults.{h,cpp}` (in zircon.core): four constexpr blobs (namespace-scope constexpr POD — rule 1a's documented exemption, read-only process-lifetime data) + the POD descriptor `zircon_embedded_default_t` (type enum, byte ptr, size, texture width/height/format) + `zircon_embedded_defaults` (the facade: static `get_embedded_default(type)` + the member loud-once + diagnostics) + the resolution seam `zircon_resolve_or_default` with the four per-type inline forwards (`zircon_resolve_texture/shader/audio/json_or_default`). BLOBS: 2×2 magenta/black checker (16 B RGBA8 pixel block + 2/2/`kRGBA8_Unorm` metadata — a pixel block, NOT a file container); the shader default is a SEMANTIC TOKEN `ZIRCON_SHADER_DEFAULT_UNLIT_MAGENTA` (35 B — the documented decision: a real blob is backend-specific and the backend is a runtime choice, so the RENDER side maps the token to its built-in unlit-magenta material per backend — bgfx AND the NRI path; the mapping is render-side future work, zircon core stays backend-free); silent audio (108 B canonical PCM WAV — mono 8 kHz 16-bit, 44-byte header + 32 zeroed samples); empty-json `{}` (2 B — the type that keeps config-style loads never-fail). LOUD-ONCE: one warning per (type, path) pair per instance (fnv1a-64 pair hash in a member `static_vector_t<uint64, ZIRCON_DEF_EMBEDDED_DEFAULTS_MAX_WARNED=16>`; overflow = ONE suppression notice, defaults keep resolving silently — never a fault; member-level per rule 1a, the same single-threaded-at-a-time discipline as the filesystem). SEAM CONTRACT: false only on a caller error (null args, or a REAL file bigger than the caller buffer — the filesystem's B0 too-small required-size contract, retriable); a missing/unreadable file resolves the default with `is_default=true`; a present 0-byte file is a chain hit ("empty means default" policy is the consumer's). RESOURCE MANAGER INTEGRATION (`zircon_resource_manager`): desc gained `is_default` (the editor badges defaulted resources later — recorded, not built); the manager owns a `zircon_embedded_defaults` member (+ `get_embedded_defaults()`); the B3-noted native-only `Is_Exists` pre-check (the old `zircon_resource_manager.cpp:239`) is GONE — it precluded pack-hosted resources, and existence is now judged by the dispatcher's read path (Get_FileSize + Begin_Stream walk packs per the priority machinery); the kText no-cache branch installs the embedded `{}` (through an 8-byte scratch copy — `Create_FromMemory`'s signature is non-const, the parser never writes) whenever the chain produced nothing usable (missing/empty/oversized-for-the-branch/unparseable — each reason keeps its own warning line), marking is_default=true + is_loaded=true. WHO RIDES THE CHAIN (the plan's point 5, decisions in the header): the localization manager does NOT (a missing language file keeps the previous table + key echo — strictly better than an empty-entries `{}` locale, and an empty table is not a missing one); zircon_config does NOT (its missing-file case already installs the hardcoded defaults — the designed semantic floor; `{}` would only reach the same defaults through the absent-key path with a misleading "default used" log on top). EN-ROUTE FIX: the pre-existing `#ifdef ZIRCON_DEF_RESOURCE_MANAGER_ENABLE_WORKER_THREAD == 1` typo (C4067 in every TU including the header) → `#if ... == 1` like every sibling site. TESTS (NEW `src/engine/tests/zircon_unit_tests_embedded_defaults.cpp`, 7, all lightweight rule 8a): per-type absent-everywhere → the embedded blob loads (same ADDRESS — the default is reused, never copied), the loud-once fires EXACTLY once across two resolutions, no exit/fault/assert on the path, the second resolution reuses the resolved default; the per-type negative control (a real file shadows the default — real bytes, is_default=false, warned count untouched); the too-small real file is a caller error (required size back, NOT a default); the blob content pins (checker pixels + metadata, the token bytes, the WAV header + silent samples, the `{}` bytes); `ResourceManagerMissingTextResolvesToEmbeddedDefault` (missing → is_default=true + is_loaded=true + a valid empty-json view; the second load reuses the default with the warned count still 1; the real-file leg shadows it); `ResourceManagerPackHostedTextLoadsThroughDispatcher` (the :239 gate fix — phase A the pack-only marker defaults with no pack mounted, phase B the tiny in-test pack through the shared encoder mounts at a FRESH filesystem's Initialize exactly like boot and the manager's sync load resolves the pack-hosted json through the dispatcher: is_default=false, the DOM carries the pack bytes); `EmbeddedDefaultsWarnedSetCapsGracefully` (20 distinct missing resources against the 16-entry set: all resolve, the set caps at 16, one suppression notice). VERIFIED: main + gfxdev Debug RC 0 (0 errors, the C4067 gone, no new compile warnings); boot `--no_splash --editor_imgui --kotek_frames=30` exit 0; 82/82 zircon (75+7) + 255/255 kotek green with the 7 new OK lines; zero new boot-flow warnings (every `[embedded-defaults]` line is a test's intentional one — 16+1-suppression in the capacity test, one per (type,path) elsewhere, the dedupe across repeat resolutions visible in the log); 4 debug_heap(996) residuals (≤6 tolerated); vfm balance 9/9 |
| Z24 | Renderer technique set (the approved GPU-driven renderer plan: B1 classic baseline → B2 CSG chunks → B3 nanite-on-NRI → B4 BCn textures → B5 terrain/lighting) | in-progress (2026-09-20 B1 done) | B1 DONE (2026-09-20): the GPU-driven classic baseline for chunked static geometry, as the A/B alternate of `model_static` in the game pass set (the CPU path stays the default; the live toggle is `render_passes_game_toggle_ab()` through the P2a frame-boundary rebuild, or the `render_passes_game`/`scene.json` keys directly). NEW `zircon_render_chunk_pool.{h,cpp}` (bgfx-free, in the passes project): the shared pools (ONE dynamic vertex + ONE dynamic index buffer; capacities `zircon_DEF_RENDER_CHUNK_POOL_MAX_VERTICES`=65536 / `_MAX_INDICES`=196608 / `_MAX_CHUNKS`=1024, sized by comment), a bounded free-list range allocator per pool (member per §2.1a; first-fit+split alloc, sorted-insert free with both-neighbour coalescing; on a failed fit with enough total free the pool defragments ONCE — `compute_defrag_relocations` hands old→new pairs, the CPU shadow memmoves, the free list collapses to the tail range — the documented amortized trigger; a second failure is a loud overflow error), and the AoSoA chunk tables per §2.9's DOD rule: the bounds bundle (2×float4 per chunk, 32 B, byte-exact with the GPU upload — aabb_min.xyz + the live-flag bits in the spare w lane, aabb_max.xyz + material-id bits), the narrow 12-byte ranges records (u16 vertex offset/count, u32 index offset — the index pool is deeper than 65536 — u16 index count + u16 spare; expanded to one uint4 per chunk at upload), u16 material ids (CPU-side only in B1 — no material system) and u8 live flags. Transforms are BAKED at registration (world-space positions/normals/AABB; identity = the phase's fixtures); runtime transform edits need re-registration (B2). The pass `zircon_render_graph_pass_model_static_gpu_driven_bgfx` (game registry via the factory codegen): one-shot world collect at the first rendered frame (gltf submeshes + kBox cubes; an empty world fills the synthetic grid fixture — `ZIRCON_DEF_RENDER_PASS_GPU_DRIVEN_SYNTHETIC_GRID_SIDE`=8 → 512 cubes with distinct AABBs), then per frame: extract the 6 frustum planes from view·proj (pool statics), dispatch the ONE cull kernel three times (modes via u_cullMeta.y: clear slots+counter → per-chunk AABB p-vertex test + InterlockedAdd compaction into the indirect buffer → 1-thread stats copy), blit the count, ONE `bgfx::submit` with the indirect buffer (the 32-byte command = the D3D11/Vk 5-word args + 12 pad — layout proof in the shader header: renderer_d3d11.cpp:3677, renderer_vk.cpp:9323, config.h:359's `BGFX_CONFIG_DRAW_INDIRECT_STRIDE`=32). The VS carries no model matrix (baked pools; u_viewProj is the bgfx predefined); the FS is the SHARED `zircon_evaluate_phong` from zircon_core.slang (model_static.fs.slang calls it too — never forked; the C++ `evaluate_phong` mirror still pins both). The readback hop: stats mode writes a 1×1 R32U (bgfx has NO buffer readback; COMPUTE_WRITE|READ_BACK on one texture is FORBIDDEN — bgfx.cpp:4663-4670 — so the compute-write texture blits to a READ_BACK|BLIT_DST twin, `bgfx::readTexture` picks it up; the first completed read is pipeline-warmup garbage and is dropped); the pass logs the first indirect submit + the A/B counts once + on changes. SHADER PIPELINE (compute is a new stage): `cmake/zircon_slang_shaders.cmake` gained `zircon_add_slang_compute_shader` (cs routes mirroring vs/fs: slangc spirv→pack / hlsl+`-DZIRCON_SLANG_BGFX_FXC`→fxc cs_5_0→pack / dxil); `zircon_shaderpack` gained `--type c` (CSH magic, both hashes 0, zero attrs) + storage uniform specs (`storagebuffer[_ro]`/`storageimage`: type byte End(1)[|kUniformReadOnlyBit], regIndex = the SPIR-V binding, regCount forced to the descriptor id 0x0007/0x0003) — the vulkan container carries them (renderer_vk.cpp:5138-5172 builds the descriptor layout from it), the d3d11 container omits them exactly like shaderc's hlsl route. TWO LATENT BUGS FIXED EN ROUTE: (a) the packer's pre-existing `k_magic_csh` spelled 'HSC' (0x0b435348) — dormant while dump-only; bgfx's createShader rejects it via isShaderBin (bgfx_p.h:437) → corrected to 0x0b485343; (b) `bgfx::setUniform` defaults `_num=1` — the six-plane array uploaded ONE element (the one-plane cull read 353 vs the mirror's 131) → setUniform(planes, 6). The bx matrix-convention lesson (recorded in the pool header): bx stores row-major with row-vector math (M[r][c]=m[r*4+c], translation at [12..14]) and the shaders' `column_major` mul() reads the transpose-view of the same bytes — plane extraction reads columns STRIDED (component i of column j = m[i*4+j]); an earlier row-read build passed the identity-matrix test by symmetry and broke the far plane. TESTS (NEW `src/engine/tests/zircon_unit_tests_chunk_pool.cpp`, 6, headless): allocator fits/free-list reuse/coalescing/overflow-rejection, the defrag relocation plan + post-defrag refit, identity view-proj plane extraction pinned byte-exact, the 6-case frustum classification (fully in / fully out per plane / straddling / degenerate point AABB / camera inside), registration + the synthetic grid under a known camera (expected visible ids+count, removal drops the chunk, slot recycling, the bake pinned against `compose_model_matrix`), loud overflow (chunk-table cap + vertex-pool defrag-can't-help rollback). The GPU-vs-CPU equivalence (the plan's (d)) is boot-evidence, not gtest — the suite runs at module init, before any frame exists; the game-session boot `--no_splash --kotek_frames=40 --exec="render_passes_game_toggle_ab()"` logs `[model_static_gpu_driven] first indirect submit: 512 chunk slots (512 live), cpu-mirror visible 131, 1 indirect submit x 512 command slots` and `A/B readback: gpu-visible 131 vs cpu-mirror 131 — match`; a standalone bgfx repro (scratch, deleted) reproduced the whole hop with 2 chunks (expected 1, got 1). VERIFIED: main+gfxdev Debug RC 0 (0 errors); plain boot exit 0, 88/88 zircon (+6) + 263/263 kotek, 4 debug_heap(996) residuals (≤6 tolerated). VULKAN NOTE: the vk container + storage table are built and dump-verified, but the boot path exercises d3d11; bgfx's vk renderer inserts no barriers between dispatches (the clear→cull→stats ordering is API-guaranteed on d3d11) — a bgfx-side gap to revisit when the vulkan route becomes active. REMAINING (B2+): the world collect is one-shot (re-collect via pass rebuild — the toggle/hot-reload does it); per-chunk `start_instance` is written but unread by the draw (d3d11 doesn't expose StartInstanceLocation to the shader; baked transforms + the single shared material make it unnecessary this phase); the editor stays on the dynamic path by design; the `--editor_imgui` boot never draws the game graph (only the CURRENT graph draws — the A/B evidence boots without it); the game graph never draws while the editor session is current. B3a DONE (2026-09-23): the nanite path's CONTENT side (NO GPU work this phase — the game-side loader + GPU cluster culling + per-cluster LOD selection are B3b). NEW `src/core/zircon_meshlet_clusterize.{h,cpp}` (zircon.core, pure content): one triangle soup in (positions + per-triangle normals + indices + per-triangle materials — the SAME shape the CSG bake emits; the LE field codec + the u8 octahedral normal codec are REUSED from zircon_csg_bake.h — one codec home, never forked), the clusterized LOD hierarchy out, written through the shared kpack encoder. THE CLUSTERIZER: LOD0 = seeded region growth over the shared-vertex adjacency graph — a LEVEL-GLOBAL FIFO frontier (the meshlette discipline: the frontier survives the cluster close, the next cluster grows from the previous boundary) with a per-cluster HOLDING retry (a vertex-budget misfit is re-offered to the next cluster at close, never lost); candidates are breadth-first in discovery order (unweighted adjacency hop = the v1 "geodesic-ish" metric, ties by mesh index); budgets `ZIRCON_DEF_MESHLET_MAX_TRIANGLES`=124 (the nanite-converged size) / `ZIRCON_DEF_MESHLET_MAX_VERTICES`=62 (the classic budget — keeps the LOD0 indices in u8, the budget's reason); a dry frontier refills by rescanning the cluster's vertices through RESUMABLE per-vertex cursors (each vertex's adjacency list is walked once per level — the amortized linear bound); region growth over a strip order for compactness (tight cull bounds + LOD parent locality — the nanite argument, documented). THE LOD HIERARCHY (the DECIMATION-FREE placeholder — the real simplifier is explicitly deferred): level L+1 = the GREEDY ADJACENCY PAIRING of level L's clusters (each pairs with its first unpaired vertex-adjacent neighbor, fallback = the next unpaired in emission order) — the exact-halving re-grouping at the doubled budget (124<<L / 62<<L) with the geometry UNCHANGED (the per-cluster error_metric field is the SIMPLIFIER'S SLOT, 0.0f this phase — a real simplifier fills it); the level table caps at `ZIRCON_DEF_MESHLET_MAX_LOD_LEVELS`=6; a mid-hierarchy cluster-table overflow TRUNCATES the remaining levels loudly (the pack stays loadable). THE NORMAL CONE (the nanite backface trick, derived + pinned): axis = the normalized mean of the member normals, cutoff = sin(theta) of the largest deviation; culled iff dot(view_direction, axis) > cutoff (d = the unit view direction into the scene; a >= 90-degree span or a degenerate mean = the never-cull sentinel cutoff 2.0f) — the predicate lives content-side as `zircon_meshlet_cone_culls` (runtime use is B3b). THE PACK FORMAT (field-by-field in the header, the single source of truth): `meshlets/<scene>/manifest.bin` (magic ZMSHLET1; level table level-ASCENDING; 88-byte cluster records: counts, the child-link run, the error slot, the cone, the exact f64 AABB, the material span) + `meshlets/<scene>/lod_<l>/cluster_<i>.bin` (u16 positions quantized against the CLUSTER's own AABB — the same fixed round-half-up rule + half-LSB bound the CSG bake documents; u8 octahedral per-triangle normals; u8 indices at LOD0 / u16 above, a deterministic function of the level; per-triangle u16 materials); the pack ENTRY order is level-DESCENDING (the coarsest level first — the progressive-loading posture; the camera-path refinement order is runtime state, not pack order). CAPS: total clusters per scene 4090 (the pack reader's entry budget; a LOD0 overflow is a HARD failure — no file written; mesh caps derived: 253,580 triangles / 760,740 vertices), the 64 MB entry-resident payload arena (the encoder's contract, the CSG-bake precedent). DETERMINISM (the csg-bake contract — content goes into packs with hashes): every decision index-ordered, IEEE double/float with fixed evaluation order, the deterministic encoder — the test bakes twice and memcmps. TESTS (NEW `src/engine/tests/zircon_unit_tests_meshlet.cpp`, 8, all lightweight rule 8a): disconnected quads = exactly one cluster per quad (no adjacency = no merging) + per-level triangle conservation; the welded 8×8 grid = 2 compact clusters (95+33, the 62-vert budget binds — the exact count pinned as the deterministic-algorithm signature) + vertex-connectedness + exactly-once coverage + exact AABBs; the cone backface math (the flat quad rejects the away hemisphere, the open cube corner pins axis (1,1,1)/√3 + cutoff √(2/3), the closed cube gets the never-cull sentinel over a 64-direction sweep); the LOD hierarchy (63 disconnected triangles → 6 levels halving exactly 63,32,16,8,4,2 to the cap; the links cover every leaf exactly once; parents = the exact child-geometry union, error 0); the pack roundtrip (bake → fresh-env mount → manifest + every bin through the dispatcher, byte-consistent with the in-memory hierarchy, the deterministic weld bijection pinned per bin, positions within the half-LSB bound, materials conserved); the streaming order (the entry table coarsest-first); determinism (two packs memcmp); the capacity guards (4091 clusters = loud hard failure + no file, the path-walk scene name + the over-cap mesh rejected). THREE FIXES LANDED DURING BRING-UP: (a) the level table recorded each level's END instead of its START (the manifest's level table was shifted — caught by the exact-count pin); (b) the first growth design (per-cluster frontier + deferred-seeds + per-cluster cursor reset) STARVED after the first cluster — the add-time scan consumed every vertex list once, making later clusters' boundaries invisible; the level-global FIFO + holding retry replaced it (documented in the header); (c) the test fixtures' normals were written with a per-corner stride into per-triangle arrays (stack corruption, caught by the Run-Time Check — the fixtures' layout, not the clusterizer). VERIFIED: main + gfxdev Debug RC 0 (0 errors, zero new warnings — the 4 C4273s in main_game_dll.cpp are the pre-existing class); boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 154/154 zircon (92 Game incl. the 8 new OK lines) + 263/263 kotek, 4 debug_heap(996) residuals (the ≤6 baseline), zero Run-Time Check failures, `data_game/packs` cleaned by the fixtures. REMAINING (B3c+): GPU cluster culling + the LOD cut selection on NRI (bgfx has no mesh shaders — the nanite path is NRI-first per the plan), the real simplifier behind the error slot, multi-mesh scenes (one pack per mesh today — a later phase adds mesh ids to the manifest), the scene camera/lighting contract on the NRI path (B3b draws through a fixed debug orbit + a hardcoded lambert). B3b DONE (2026-09-23): the geometry seam + the FIRST REAL NRI DRAW — one baked LOD0 cluster, no culling (B3c owns culling/LOD/the toggle). THE SEAM (kotek-side, see the K11 row): `ktkIRenderGeometryManager` (device-level buffers/pipelines, opaque generation-tagged handles, immediate HOST_UPLOAD uploads) + the frame-level draw commands on `ktkIRenderFramePassContext` (Begin/End_Render_Pass, Set_Pipeline/Set_Push_Constants/Set_Vertex_Buffer/Set_Index_Buffer/Draw_Indexed, the back-buffer size) — the Z5 boundary rule holds: NO NRI types in zircon, stale handles are loud no-ops. THE PIPELINE CHOICE (documented): the vendored NRI v180 implements mesh shaders internally but its public CoreInterface exposes no mesh dispatch (vendored stays byte-pristine) — the honest v1 is the CLASSIC indexed draw of the cluster's triangles through the same seam; a Draw_Meshlets lands with an NRI version bump (B3c), the seam shape unchanged. THE READER: NEW `src/core/zircon_meshlet_cluster_read.h` (zircon.core, pure statics — the B3a format's runtime consumer): the manifest header/level-table/cluster-record parse, the per-cluster bin dequantization (the shared codecs) and the SOUP EXPANSION to the interleaved 24-byte [pos|normal] vertex layout + the identity u32 IB (welded positions can't carry per-triangle flat normals — the editor CSG pool's exact discipline); every skew is a loud false, never a crash (user content). THE PASS `no_streaming::zircon_render_graph_pass_meshlet_cluster_nri` (the NRI passes project, registered in the hand-written passlib — count 2): reads `meshlets/boot/` (scene name `ZIRCON_DEF_RENDER_PASS_MESHLET_CLUSTER_NRI_SCENE`) through the filesystem dispatcher (packs-first/native — the chunk-pool load's shape), creates the VB/IB + the pipeline through the seam (the Slang dxil route's raw blobs, a 64-byte push-constant u_viewProj block at register(b0) both stages — D3D12 root constants), records ONE indexed draw per frame ON TOP of the present pass's clear (the LOAD render pass — the default set orders present FIRST, meshlet SECOND). The pass gets its services through the NEW `zircon_nri_passlib_initialize` seam (the base class `zircon_render_graph_pass_nri::Initialize_Nri`, called by the game manager right after create — the C-ABI create-by-name never carried arguments); holds non-owning pointers + handles only (the reload-safety laws); a missing manifest/blobs/backend leaves it LOUD-INERT, never a boot failure (user data is not a programmer error). THE CAMERA/LIGHT: no scene camera on the NRI path yet — a fixed debug orbit frames the cluster AABB; the FS is a hardcoded directional lambert (the shared Phong contract needs the descriptor seam — the material/lighting phase). SHADERS: `meshlet_cluster.vs/fs.slang` (the push-constant cbuffer carries no [[vk::binding]] — the NRI route is the only live consumer; the bgfx routes build the same pair for pipeline uniformity, nothing binds them). THE SHIPPED BOOT PROBE: `meshlets/boot/{manifest.bin, lod_0/cluster_0.bin}` at the ENGINE ROOT (the pack entry namespace is root-relative — the B3a writer's contract, so a pack carrying the same entries resolves identically through the dispatcher) — a hand-generated unit cube (8 welded corners, 12 triangles, per-face normals, the never-cull cone sentinel), 128 + 152 bytes, so the NRI boot really rasterizes; regeneration: the documented B3a field layout (the bytes are pinned by `Zircon_NriMeshlet.BootFixtureLoadsThroughDispatcher`). TESTS: kotek `KotekRenderNRI.GeometryHandleTableBookkeeping` (the handle table headlessly: generation reuse, stale/foreign guards, the capacity + upload-range contracts) + NEW `src/engine/tests/zircon_unit_tests_meshlet_nri.cpp` (6: the soup layout byte-pinned incl. the half-LSB dequant bound, the corrupt-content rejection guards, the manifest parse pins, the entry-name builder + path-walk rejection, the shipped fixture through the REAL dispatcher, the passlib registry/lifecycle incl. the null-manager inert contract). VERIFIED: main + gfxdev Debug RC 0; the default bgfx boot exit 0 (the NRI pass is inert there by construction); the NRI boot `--no_splash --render_nri_dx12 --kotek_frames=30` exit 0 with `[render]: game render pass set resolved from the built-in default: no_streaming::zircon_render_graph_pass_present_nri,no_streaming::zircon_render_graph_pass_meshlet_cluster_nri`, the pass's own `[nri meshlet] pass created: cluster 'boot' LOD0 cluster 0 = 12 triangles (8 welded vertices -> 36 soup vertices), VB 864 B + IB 144 B uploaded, pipeline created (vs+ps DXIL, 64 B push constants)` and `[nri]: frame passes driven: 2 (...)` — see the final report for the quoted lines; suites green with the new OK lines; ≤6 residuals. ONE BRING-UP BUG THE BOOT CAUGHT: the NRI builtin default's first draft spelled the concatenation as `name_a "," name_b` over two `const char*` variables — ill-formed (adjacent-literal concatenation needs literals); the house form is the kZirconConfig literal style — fixed in the header. TWO SHUTDOWN-ORDERING DEFECTS THE BOOT CAUGHT (both fixed): (a) NRI's validation layer requires `CmdSetPipelineLayout` BEFORE `CmdSetRootConstants` — the context now binds the layout alongside `Set_Pipeline` (the root-constant index is local in the bound layout); (b) the NRI passes hold geometry-seam handles into the NRI device, but `ShutdownModule_Game` invokes the render module shutdown (which destroys the device + the geometry manager) BEFORE `g_main_manager.Shutdown` — the passes died on a dead device (segfault); the fix is the EARLY teardown `zircon_game_manager::Destroy_Nri_Frame_Passes()` (GPUFlush FIRST — destroying in-flight resources upstream of the module shutdown's `DeviceWaitIdle` wedged the fence wait; then the passlib destroys, then the renderer's views are unseated), called from `ShutdownModule_Game` ahead of the render shutdown invoke; the later teardown sites re-enter on the cleared list harmlessly. RESIDUAL NOTE: the NRI boot's debug_heap(996) cross-CRT markers measure 9 (5 = the shared boot baseline, 2 = the NRI backend's own teardown — present with ONLY the clear pass, 2 = the B3b geometry path) — all the single documented cross-CRT class (exit 0; the real cure is K9) |
| Z25 | CSG (brush-based constructive solid geometry, the approved Part A plan) | done (A3 done 2026-09-22 — the CSG ARC COMPLETE) | A1 DONE: the precision-configurable scalar + the component pair + the plane-set evaluation core + 35 test proofs (typed helpers × precision modes). `zircon_csg_defs.h`: `ZIRCON_DEF_CSG_PRECISION` = F32 (default) / U32F (u32 16.16 two's-complement fixed-point — INTEGER math = identical bits on every platform/CRT/optimization level: no FPU rounding modes, no FMA contraction, no x87 — deterministic input = deterministic output, the bake-reproducibility proof) / F64 (robustness debug); #error guard; u32f overflow NEVER silent/saturating (loud warning + two's-complement wrap — a content error stays visible); range contract ±16384 documented; all three modes instantiate in EVERY build (parity/determinism tests run everywhere; the define selects what the COMPONENTS store). `zircon_csg_evaluate.h` (render-free pure math, templated on the scalar): the Quake/Valve brush model — a primitive is a convex plane set + polygon soup, booleans classify+clip on PLANE SETS (union keeps outside fragments; subtraction keeps A-inside-B + B-inside-A flipped; intersection keeps mutually-inside), welded triangle mesh out (weld at the eps grid keyed to the precision mode). REJECTED FOR V1 (documented): arbitrary-mesh CSG (Cork/CGAL — heavy dep, non-deterministic), voxel/dual-contouring (memory + not brush-native), Lengyel stencil-GPU CSG (preview trick, no mesh). COMPONENTS: `zircon_component_csg_primitive` (type box/wedge/cylinder, 3-scalar dimensions, material_id u16, op flags u8 with the subtractive bit, own position+rotation) + `zircon_component_csg` (bounded member-id list, dirty u8 flag, evaluated-mesh handle POD), registered in the codegen'd enum + the manual factory switch + json tag_invokes (both appear in the inspector combo via the existing ui_state fill). TESTS (`zircon_unit_tests_csg.cpp`, 35, headless): u32f exact-arithmetic pins; box/wedge brush construction exact; every operation × every precision mode via typed helpers; welded-mesh invariants (watertight = every edge exactly twice, signed-volume pins — the union expectation settled by VOLUME + watertightness, not a hand-counted face number); weld edge cases (T-junction seam vertices, near-parallel planes at 1e-4 rad — deterministic, slivers policy-dropped by design); sliver-drop policy (eps-shallow overlaps → empty + counted, never silent); capacity rejection (loud graceful); **DETERMINISM: two u32f runs byte-identical**; precision parity f32 vs u32f within the documented band; component json roundtrips + the factory proof. Verified (parent, own boot): 123/123 zircon (88+35) + 263/263 kotek, boot exit 0, 4 documented residuals. A2 + A3 LANDED (below) — the CSG ARC IS COMPLETE. A2 DONE: the editor CSG integration (the exact A2 scope — the game bake is A3, not here). COMMANDS (NEW `src/editor/commands/zircon_command_csg_{create,delete,edit}_primitive.{h,cpp}` + the header-only `zircon_command_csg_utils.h`, registered in the command registry as zircon-local ids 0x80000002-4): the create command carries the primitive's serialized component json as its payload (the factory's json-create installs the whole component — the same state the journal roundtrips; the UI-facing ctor takes typed params and builds the json); the delete command captures the primitive state at Execute (the delete-entity capture pattern), unregisters + destroys, Undo recreates + re-registers (the fresh incarnation rides the history's GetEntityID observation; the recorded-id re-targeting is execute_node's SetEntityID contract — mirrored explicitly in the delta test); the edit command is the edit_component_state delta shape (before+after json) + the owning compound id. ALL THREE mark the COMPOUND dirty on execute AND undo/redo (the rebuild keys on dirtiness, not the command kind) and resolve the compound through the history's reincarnation chain (`get_live_entity_id`, the only non-primary id — the history translates only the GetEntityID primary). SCHEDULER (NEW `src/editor/session/zircon_csg_editor_rebuild_scheduler.{h,cpp}`, one per editor session like the command history, started in `initialize`/joined in `shutdown`): the dirty watch is a bounded tracked-compound table (64, loud-drop class); TWO triggers — the journaled-batch boundary (any movement of the (recorded-count, cursor-node) epoch the session computes per frame — a completed command or an undo/redo — dispatches every waiting compound IMMEDIATELY) and input quiet (a dirty compound seen WITHOUT a history move is a live/direct edit; it debounces `ZIRCON_DEF_CSG_EDITOR_REBUILD_DEBOUNCE_MS`=150 of the INJECTED fnptr+void* clock — a test advances a counter, wrap-safe signed elapsed math, no sleeping); a re-dirty inside the window resets the timer; an edit while queued/in-flight raises the re-dirty flag → exactly ONE trailing debounced rebuild after the result is consumed (a command-per-frame slider burst coalesces into a first immediate + one trailing rebuild); the dirty flag clears AT dispatch (the job snapshot is the source of truth). The evaluation runs on the scheduler's OWN worker (ONE bounded queue of 8 jobs × the member-list snapshot, single consumer; NOT the resource manager's B3 worker — that one is a private fixed-shape text-load queue, documented) with ONE completion slot: the consumer (the editor CSG pass, render thread) checks the result out BY VIEW (zero-copy pointers into the ~2.3 MB slot — never copied) and returns it; an un-consumed slot backpressures the worker (a stalled frame stalls rebuilds, never the reverse). The ~6 MB evaluation context + the slot are heap-only (the fixture rule). POOL (NEW `src/render/bgfx/passes/no_streaming/zircon_render_csg_editor_pool.{h,cpp}`, bgfx-free like chunk_pool): ONE dynamic vertex pool + ONE dynamic index pool for ALL editor compounds (196608 verts/indices = 3× the eval triangle cap — the mesh is a TRIANGLE SOUP: welded positions can't carry per-triangle flat normals, the expansion happens at upsert); pool-global u32 indices (one draw call, BGFX_BUFFER_INDEX32); zircon_pool_range_allocator REUSED from chunk_pool.h (never forked; its chunk-pool free-list capacity amply covers the smaller table); freed index spans re-zeroed to degenerate (0,0,0) triangles so the holes between compounds rasterize nothing inside the single draw (vertices of freed spans stay — unreferenced bytes); defrag = the chunk-pool amortized trigger (failed fit + enough total free → compact-once + retry; freed index tail re-zeroed; full-range re-upload), span bookkeeping degrades to one full-range span per pool on overflow; an upsert writes ONLY the rebuilt compound's ranges (BYTE-PINNED — a test memcmps the untouched compound). PASS (NEW `zircon_render_graph_pass_editor_csg.{h,cpp}` — the factory codegen picks it up; joined into the DEFAULT editor set between grid and gizmo, `src/core/zircon_config.h` + the tracked `data_user/game_config.json`): per frame drains the scheduler → converts scalar→float in a bounded scratch → pool upsert → reconciles the pool against the world (a compound that died through a NON-csg command releases its range) → bgfx::update of the CHANGED spans only → ONE submit with the shared zircon_evaluate_phong (editor_csg.vs/fs.slang — the model_static_gpu_driven contract verbatim: ModelParams u_viewProj predefined cbuffer, the LightParams b1/b0-fxc discipline); an empty pool is a no-op. SHADERS: `zircon_add_slang_shader(editor_csg ...)` in cmake/zircon_slang_shaders.cmake (the pipeline's incremental rebuild picks the pair up). TESTS (NEW `src/engine/tests/zircon_unit_tests_csg_editor.cpp`, 12, all lightweight rule 8a): the three commands through the REAL history (execute/undo/redo state + the dirty flag at every boundary), the journal delta roundtrips (serialize → deserialize → re-execute to the same observable state, the delete leg pinning the recorded-id re-targeting), the debounce on the injected clock (quiet 150 ms → ONE rebuild; a re-edit resets the timer; the boundary fires immediately; the in-flight edit coalesces into one trailing rebuild), the REAL worker handoff (condvar wait, no sleeping), the end-to-end session drive (journaled command → session update → worker → completed box mesh = exactly 12 triangles), the pool matrix (ranges + dirty spans, the byte-pin, the degenerate-hole remove + high-water reset, the defrag compaction + full-range degradation, the overflow loud graceful), the pass registration (the generated registry carries the name, count 7, the factory creates by name). THREE EN-ROTE FIXES THE BOOT EXPOSED: (a) the compound's dirty flag became a GENERATION COUNTER (`mark_dirty()` bumps a u8, nonzero = dirty — a level flag could not express "edited again while still dirty", which made the debounce timer restart every frame and never fire); (b) `zircon_resource_manager`'s async publish order — `desc.is_loaded` was written BEFORE the view placement-new, publishing "loaded" with a null view to the spin-waiting consumer (a latent race the scheduler's worker thread widened into a deterministic abort; the flag is now the LAST write, the contract is_loaded ⇒ view valid); (c) `ZIRCON_DEF_CONFIG_RENDER_PASS_LIST_MAX_LENGTH` 256 → 512 (the 5-pass default editor set measures 287). A POOL BUG THE DEFRAG TEST CAUGHT: the allocator emits one relocation per live SEGMENT (many records share it) — the record rewrite matches the entry CONTAINING each record's old offset and shifts by its delta (a segment-start-only match silently stranded mid-segment records). VERIFIED: main + gfxdev Debug RC 0; boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, `[editor_csg] pass created: one dynamic pool (196608 vertices / 196608 indices)` + the executor's `pass[2]` line, 139/139 zircon (127+12) + 263/263 kotek green, zero new warnings, 4 debug_heap(996) residuals (≤6 tolerated); the boot world has no compounds so the pass proves creation + the empty-pool no-op (the draw path is boot-evidence-only discipline like B1; the full chain command → scheduler → worker → evaluated mesh is test-proven, the pool merge is byte-pinned test-proven). A3 DONE 2026-09-22 (below) — the game bake + the pack-loaded read path; the CSG ARC IS COMPLETE. BAKE (NEW `src/core/zircon_csg_bake.{h,cpp}`, zircon.core, pure content, no render): `zircon_csg_bake_pack(fs, pack_path, compounds, scene_name) -> bool` evaluates every compound through the EXISTING A1 core (never reimplemented — one reused heap context, the ~6 MB fixture rule) and writes the pack through the SHARED encoder `kpack_write_file` — entry layout `csg/<scene>/manifest.bin` (32 B header + 96 B records: cells, counts, the PRE-quantization f64 per-chunk AABBs, the entry-name hash — the load-side integrity check) + `materials.json` (the sorted distinct material ids) + `compound_<i>/chunk_<j>.bin` (16 B header + the quantized tables); the format is documented field-by-field in the header (the single source of truth for BOTH sides). THE CHUNKING (documented rule, test-pinned): each triangle lands in EXACTLY ONE chunk — the grid cell of its CENTROID, cell = floor(coord / `ZIRCON_DEF_CSG_BAKE_CHUNK_SIZE_METERS`=16), half-open cells, a boundary centroid lands in the HIGHER cell (no seam duplication; a straddler draws whole from its owning chunk; per-chunk AABBs exact from the member triangles; seam vertices duplicated per chunk by design). THE QUANTIZATION (the plan's u8/u16 GPU format): positions u16 normalized per chunk against its AABB — the half-LSB bound extent/131070 ≈ 0.12 mm on a 16 m cell (u8 under `ZIRCON_DEF_CSG_BAKE_POSITION_QUANT_BITS`=8, the documented small mode, ~31 mm); normals u8 OCTAHEDRAL PER TRIANGLE (2 B vs the u16-per-component's 6 B — flat-shaded brush faces don't need more; conservative angular bound 0.015 rad pinned over a 512-direction Fibonacci sweep incl. the fold seams); indices u32 chunk-local; per-triangle u16 materials. THE STREAMING ORDER: grid order — cells lexicographic (x, y, z), compounds in input order; a fixed function of the geometry (a camera-path order is content-dependent). THE DETERMINISM CONTRACT (pinned by `CsgBakeDeterminism`): two bakes produce BYTE-IDENTICAL packs — the A1 u32f evaluation (bit-exact everywhere) + integer/DOUBLE bake math with fixed rounding rules (no FPU-dependent steps) + the fixed order + the deterministic encoder (zstd at a fixed level, no timestamps). CAPS named + loud: per-compound 64 cells (a 1 km row — the compound is SKIPPED gracefully, the bake continues), scene 4090 chunks (the pack reader's entry budget — a hard stop: the bake never emits an unloadable pack), per-chunk 21845 triangles (the pool's u16 vertex count — a compound that could never load is skipped at bake time), 64 MB entry-resident payload (the encoder API holds every entry until the write returns — a streaming encoder is the documented future work if levels outgrow it). GAME-SIDE READ PATH (`zircon_render_chunk_pool::load_chunks_from_pack(fs, "csg/<scene>")` — the B1 pool's pack-loaded variant): the manifest + chunk entries read through the filesystem DISPATCHER (the packs-vs-dirs priority chain, cwd-independent root resolution), manifest magic/grid/count/record validation + the name-hash skew check + a FULL capacity pre-check (the triangle-soup expansion costs 3 pool vertices + 3 indices per triangle) BEFORE any registration, then dequantize (positions in double against the manifest's f64 bounds → float at the pool boundary; normals oct → float) and register as B1 chunks for the GPU-culled path — the game session runs ZERO CSG evaluation. A console command was DELIBERATELY not added: a bake needs compound records from a live world traversal — its caller is the future level-compile step (the tests drive the function directly). TESTS (NEW `src/engine/tests/zircon_unit_tests_csg_bake.cpp`, 7, all lightweight rule 8a): the 3-cell grid split (manifest cells + exact f64 AABBs, 36 triangles conserved), the exact-boundary rule (the centroid at x=16 lands in the higher cell, 10+2 = 12), the quantization bounds (the half-LSB pin, the degenerate flat-axis dequant, the octahedral sweep), the full pack roundtrip (bake → pack → fresh-env mount → dispatcher → load → pool state == the pre-bake mesh within the bound + the cull mirror sees the chunk), determinism (the two packs memcmp), the streaming order (the pack's entry table hashed in the documented order), the capacity guards (a 65-cell compound skipped loudly while the valid compound bakes + loads; the scene-name path-walk rejected). TWO TEST-HARNESS LESSONS: the fixtures must sit INSIDE single grid cells (a box straddling the y=0 plane spans 4 cells — the first run's 12-chunk failure was the fixture, not the code) and the pack tests remove the `data_game/packs` FOLDER with the last pack (the shipped tree has none — the embedded-defaults fixture pins that invariant; the second run caught it). VERIFIED: main + gfxdev Debug RC 0 (0 errors, zero new warnings); boot `--no_splash --editor_imgui --kotek_frames=30` exit 0, 146/146 zircon (139+7) + 263/263 kotek green, 4 debug_heap(996) residuals (the ≤6-tolerated baseline class — one transient 5th residual on an intermediate fixture was heap-layout perturbation, gone with the final fixtures), zero new warnings (the one `[csg_bake]` capacity line is the test's intentional loud-skip). |
