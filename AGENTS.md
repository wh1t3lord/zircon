...
| Z7 | Enforce static-container/`ktk`-alias rule consistently across `src/` | in-progress (2026-07-25 batch 4) | batches 1-4 done — full re-audit in §5 "Container-rule violations": active `src/` is clean of dynamic `vector_t`, raw `std::thread/lock_guard`, dead duplicates, and raw `char[]` (batch 4: 8 scratch buffers → `kotek::array_t` — ui_top_bar/object_list/history_log/debug_input/commands×2/config read buffer; one site reverted+annotated exempt: `zircon_config.cpp` serialize passes a raw array to kotek's template `Serialize_ToString(char(&)[N], Size&)`); two std::string_view sites are kotek-interface-forced (annotated). REMAINING: ENTT-guarded `vector_t` (with the ENTT backend task), `zircon_config.h` redesign — `translate_zircon_game_features` returns dynamic `ktk::cstring` by value (cross-module hazard class, §5) + `m_features_data_*` are dynamic `unordered_map<variant<...,cstring>>` (rule-2 redesign: lookup table on static_vector + static_cstring_t, batch 5) |
