...
| Z5 | Passes for editor AND game, for bgfx AND NRI; NRI gets own folder, same two-project split (passes + executor) | in-progress (2026-07-25 phase 1) | kotek K11 phase 1 landed: `src/render/nri/zircon_renderer_nri.{h,cpp}` — minimal `ktkIRenderer` over the kotek.render.nri swapchain (clear-color present; **no NRI types in zircon**), DirectX-slot cases in every `zircon_game_manager` renderer switch, sessions skip render-graph init (invalid id), select with `--render_nri_dx12`. Verified: boot exit 0, 30 frames presented, clean shutdown. REMAINING (phase 2): the two-project split (hot-swappable passes lib + executor) mirrored for NRI, actual passes for editor+game on both backends |
...
| Z13 | CI/CD green on GitHub + three linkage configurations in CI | in-progress (2026-07-31 public flip) | kotek side fully green (K5: build/tests/modules on 78cbd58). **zircon is PUBLIC since 2026-07-31** (owner decision — private-repo Actions blocked: pre-flip runs sat queued for days with zero output; the first post-flip execution failed every leg in 2s with the check-run annotation "The job was not started because recent account payments have failed or your spending limit needs to be increased" — an account-side billing flag, repo-agnostic in effect; kotek's public runs were unaffected). Slim-leg redesign landed for the quota era and stays (push = Debug × {default, static}; full matrix nightly+dispatch). build.yml compiles the THREE configurations the owner requires (default / full static / full dynamic-continue-on-error per K18) × Debug/Release nightly; tests.yml runs the real engine boot (`--no_splash --kotek_frames=30`, `KOTEK_ASSERT_STDERR_ROUTING=ON`). Hardening (mirrors kotek K5): `restore-keys` vcpkg cache fallback, configure retry ×3 with tee'd `configure-attempt<N>.log` + `build-output.log`, `contents: write` + clone-based `ci-logs` publishing on failure in both workflows (the public read path — job logs/artifacts are admin-only), outcome-driven per-leg badges on the `badges` branch (shields endpoints; `merge-multiple` + loud no-json guard — the two bugs that kept every badges branch from ever existing), timeouts 180 | kotek side fully green (K5: build/tests/modules on 78cbd58); zircon workflows aligned and healthy (no failure publishes on the `ci-logs` branch). build.yml compiles the THREE configurations the owner requires (default / full static / full dynamic as continue-on-error per K18) × Debug/Release; tests.yml runs the real engine boot (`--no_splash --kotek_frames=30`, `KOTEK_ASSERT_STDERR_ROUTING=ON`). Hardening (mirrors kotek K5): `restore-keys` vcpkg cache fallback, configure retry ×3 with tee'd `configure-attempt<N>.log` + `build-output.log`, `contents: write` + clone-based `ci-logs` publishing on failure in both workflows (the public read path — job logs/artifacts are admin-only), timeouts 180 |
