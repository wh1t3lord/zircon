...
| Z7 | Enforce static-container/`ktk`-alias rule consistently across `src/` | in-progress (2026-07-25 batch 3) | batches 1+2 + batch 3 done — full re-audit in §5 "Container-rule violations": active `src/` is clean of dynamic `vector_t`, raw `std::thread/lock_guard`, and dead duplicates; two std::string_view sites are kotek-interface-forced (exempt, annotated); remaining: ENTT-guarded `vector_t` (with the ENTT backend task), raw `char[]` audit in commands |
